"""
Backward-compatible entry point for the SignWriting-to-text setup path.

Delegates entirely to general_training_setup.main with
default_dataset_type="signwriting2text" so that configs that predate the
data.dataset_type field continue to work when --modality signwriting2text is given.

For new projects, use multimodalhugs-setup without --modality and add
  data:
    dataset_type: signwriting2text
...
def main(
    ...
    _general_main(
        ...
        default_dataset_type="signwriting2text",
    )
