"""
Backward-compatible entry point for the image-to-text setup path.

Delegates entirely to general_training_setup.main with
default_dataset_type="image2text" so that configs that predate the
data.dataset_type field continue to work when --modality image2text is given.

For new projects, use multimodalhugs-setup without --modality and add
  data:
    dataset_type: image2text
...
def main(
    ...
    _general_main(
        ...
        default_dataset_type="image2text",
    )
