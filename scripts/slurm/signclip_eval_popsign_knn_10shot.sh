...
PYTHON_BIN="${PYTHON_BIN:-/home/faxu/multimodalhugs/.pixi/envs/default/bin/python}"
...
if [[ ! -x "${PYTHON_BIN}" ]]; then
  echo "[knn10] Python executable not found: ${PYTHON_BIN}" >&2
  exit 1
fi

echo "[knn10] host=$(hostname)"
echo "[knn10] python=${PYTHON_BIN}"
echo "[knn10] dataset=${DATASET_DIR}"
echo "[knn10] models=${PORTABLE_ROOT}"
echo "[knn10] output=${OUTPUT_DIR}"

"${PYTHON_BIN}" scripts/evaluation/signclip_few_shot_knn.py \
